...
export class AlternativeSchemaComponent {
  ...
  form = ef.create({
    ...
    email: ef.text('Email',{props:{type:'email'}}).required('Email is required').email('Email is not valid'),
    phone: ef.text('Phone',{props:{type:'number'}}),
    userType: ef.select([
      {label: 'Admin', value: 'admin'},
      {label: 'User', value: 'user'}
    ], 'User Type'),
    adminRoles: ef.radio([{label: 'Super Admin', value: 'super'}, {label: 'Admin', value: 'admin'}], 'Admin Roles'),
    publishDate: ef.text<string, Date>('Publish Date', {props:{type:'date'}}),
    gender: ef.radio([
    ...
    addresses: ef.array({
      ...
      zip: ef.text('Zip',{props:{type:'number'}}).required('Zip is required')
